...
export function CandidateProducts({ products }: { products: ProductCandidate[] }) {
  return (
    <section className="rounded-lg border border-line bg-white p-5 shadow-subtle">
      <h2 className="text-base font-semibold text-ink">Candidate Products</h2>
      {!products.length ? (
        <p className="mt-3 text-sm text-slate-500">No candidate products returned.</p>
      ) : (
          ...
          {products.map((product) => (
            <article key={product.product_id} className="rounded-md border border-line bg-panel p-4">
              ...
              {product.missing_confirmations?.length ? (
                <div className="mt-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Missing confirmations</div>
