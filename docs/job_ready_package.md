...
## 13. 最终求职材料包 Career Package

最终求职材料已整理到 `docs/career_package/`：

- `resume_project_final.md`: 可直接放入简历的项目描述。
- `interview_pitch.md`: 30 秒到 5 分钟的面试讲解稿。
- `technical_qa.md`: 技术追问 Q&A。
- `project_challenges.md`: 项目难点、解决方案和面试表达。
- `job_application_notes.md`: 投递岗位、简历写法和边界说明。

建议使用顺序：

1. 先从 `resume_project_final.md` 选择简历版本。
2. 面试前背熟 `interview_pitch.md` 的 1 分钟和 3 分钟版本。
3. 用 `technical_qa.md` 准备追问。
4. 用 `project_challenges.md` 讲工程难点。
5. 用 `job_application_notes.md` 校准岗位匹配和表达边界。

所有材料继续明确：项目使用高仿真模拟数据，是 portfolio / prototype 工程化项目，不自动报价、不承诺库存、不承诺交期、不自动发送邮件。
## 14. A8 Auth & Roles 求职亮点

A8 可以作为“从公开 demo 到业务后台”的工程化补强点：

- 增加 demo login / current user / logout。
- 增加 `admin`、`sales`、`support` 三类角色。
- 将 Knowledge Base Admin 限制为 admin。
- Review 在登录状态下记录当前用户。
- 前端提供 UserMenu 和角色感知导航。

面试表达建议：

```text
我没有把后台一直保持成公开页面，而是补了一层轻量 Auth & Role-Based Access。
